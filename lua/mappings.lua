...
-- map({ "n", "i", "v" }, "<C-s>", "<cmd> w <cr>")
map("n", "<C-e>", ":NvimTreeFindFile!<CR>", { desc = "Custom: Open file tree on current file" })
...
for i = 1, 9, 1 do
  map("n", string.format("<A-%s>", i), function()
    ...
    desc = string.format("Tabs: Move to tab %s", i)
  })
end
map("n", "<C-S-PageUp>", function() require("nvchad.tabufline").move_buf(-1) end, { desc = "Tabs: Move tab left" })
map("n", "<C-S-PageDown>", function() require("nvchad.tabufline").move_buf(1) end, { desc = "Tabs: Move tab right" })
map("n", "<A-0>", "<cmd>tabnext<Cr>", { desc = "Tabs: Next tab page" })
map("n", "<leader><tab>", "<cmd>tabnext<Cr>", { desc = "Tabs: Next tab page" })
map("n", "<A-+>", "<cmd>tabnew<Cr>", { desc = "Tabs: New tab page" })
